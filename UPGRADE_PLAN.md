...
## סטטוס ביצוע (מתעדכן עם כל שלב שנסגר — חוזה התיעוד ב-CLAUDE.md)

| שלב | סטטוס |
|---|---|
| שלב 0 — מעבר ל-Java | ✅ הושלם ברובו: 10 קבצים ב-TARGET_CLASSES (כולל Home, LauncherModel, KeyCodeEventUtil) |
| שלב 1 — בטיחות | ⚠️ חלקו מומש: החלפת לאנצ'ר בסדר הבטוח + מתג מצב-אמת + מסלול חזרה (LauncherSettings) — קיים ועובד. פריטי המשנה האחרים של השלב טרם סומנו אחד-אחד |
| שלב 6 — מסך ההגדרות | ✅ הושלם 2026-08-07, מעבר למתוכנן: עיצוב NewUI מלא זהה למערכת, היררכיה חדשה, feature bar, חשיפת הגדרות יתומות, ניהול אפליקציות כרשימת מסך, איפוסים סלקטיביים. תיעוד: `plan_research/rom_settings_design/` + הסקיל `qin-newui` |
| שלבים 2–5 | לפי המסמך להלן; טרם סומנו כסגורים |
